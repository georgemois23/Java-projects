...
public class CentralRegistry {
   ...
   public static String getInDirectFlightsDetails(Airport sourceAirport, Airport destinationAirport) {
	    ArrayList<String> indirectFlightDetails = new ArrayList<>();
	    int count = 0;

	    for (Flight flight : flights) {
	        Airport intermediateAirport = null;

	        if (sourceAirport.equals(flight.getAirportA())) {
	            intermediateAirport = flight.getAirportB();
	        } else if (sourceAirport.equals(flight.getAirportB())) {
	            intermediateAirport = flight.getAirportA();
	        }

	        if (intermediateAirport != null && !intermediateAirport.equals(destinationAirport)) {
	            for (Flight indirectFlight : flights) {
	                if (intermediateAirport.equals(indirectFlight.getAirportA()) &&
	                        destinationAirport.equals(indirectFlight.getAirportB())) {
	                    count++;
	                    indirectFlightDetails.add("[" + count + "]" + intermediateAirport.getCity() + ", " + intermediateAirport.getCode() +" airport"+ "\n");
	                }
	            }
	        }
	    }

	    if (!indirectFlightDetails.isEmpty()) {
	        return String.join("", indirectFlightDetails);
	    }
